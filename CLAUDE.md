...
7. **Hooks state only what the data proves, never infer cause — and every summary number must match the module it summarizes.** Every template-generated headline/hook must be checkable against a specific field in the race JSON. If the data can't distinguish *why* something happened (e.g. Jolpica's DNF `status` is usually just "Retired", never a mechanical cause; a Gap Trace sign-flip proves the time gap crossed but not whether it was an on-track pass, a pit-stop cycle, or a penalty), the hook must not claim it. State the count or the measured fact, not the mechanism. A summary card/hero line is a *restatement* of the deep-dive module's own numbers, never a separate computation that can drift from it or misattribute a stat to the wrong driver (e.g. the Title race card's point gap must equal `driverStandings[0].points - driverStandings[1].points`; the Rejoin Strip card's counts must equal the full module's). Before shipping a new or changed hook template: run it against every backfilled race and read the resulting headlines in one pass — a phrasing that sounds fine for one race can misfire on another (an idiom that reads as sarcastic, a threshold that fires on a routine result) — and cross-check every summary number it produces against the raw JSON, not just against the code that renders it.
...
### Milestone 4 — Glance View (v1.2)
- **Two-layer landing page.** A **Glance layer** (the new default view, always visible, sized to fit one phone screen) sits in front of a **deep-dive layer** (the complete Milestone 1–3 modules, unchanged, collapsed by default).
  - Glance layer: a one-line hero (winner + a fact from the hero rulebook below) plus five compact cards — Title race, Drama log, Heroes & Zeroes, Rejoin Strip, Gap Trace.
  - Tapping a card reveals *only* that section and scrolls to it (`prefers-reduced-motion` swaps the smooth scroll for an instant jump); a "Back to glance" link collapses it again. Tapping a notable dot on the Rejoin Strip card jumps straight to that driver's block inside the full module.
  - The deep-dive modules are moved behind cards, never altered or removed — the Glance layer is a summary in front of them, not a replacement.

- **Hook rulebook** (deterministic, template-based, no LLM; approved 2026-07-23; governed by Hard Rule 7):
  - **Hero line** — always the winner's *own* story, never another driver's stat and never a race-wide fact (those already have their own cards). Jolpica gives grid + finish position only, no margin or laps-led field: "Started on pole and crossed the line first" (grid ≤ 1) / "Started P{grid}, crossed the line first" (grid 2–5), with "— the grid didn't matter today" appended at grid ≥ 6.
  - **Title race** — lead ≤ 25 pts (one race win's worth) → "Title fight: {lead} points cover P1 and P2."; lead > 100 → "{leader} is running away with it — a {lead}-point lead."; else → "{leader} leads the championship by {lead} points."
  - **Drama log** — 0 DNFs → "Clean race — everyone finished."; 1–2 → named list; ≥3 → "{n} retirements." — no "reliability"/cause framing; Jolpica's `status` is only ever "Retired"/"Did not start" across every backfilled race.
  - **Heroes & Zeroes** — always shows both the biggest mover and biggest drop with their real numbers (never omits a side). ±4 grid-to-finish places is the "worth naming" bar; below it, a neutral qualifier is appended ("a quiet day up front" / "nothing dramatic") instead of hiding the fact or dramatizing it. DNFs are excluded from eligibility by the pipeline (`eligible_for_movement`), not by the card.
  - **Rejoin Strip** — "Rejoin clash" means the pitted car came out *ahead*, defending against a rival right behind — phrased as "rejoined right ahead of a rival" (get the direction right; it's the opposite of "dirty air", which is a real broadcast term and needs no rephrasing). Dot strip shows notable (clash/dirty) stops individually, capped at 14 (one row) with overflow folded into "+N more clash/dirty" pills; clean stops always collapse to a single count.
  - **Gap Trace** — states only that the cumulative gap flipped sign ("{A} and {B}'s gap flipped around Lap {L}") — never that it was an on-track pass, which can't be distinguished from a pit-stop cycle; falls back to "Compare any two drivers' pace lap by lap" if no flip.

- **Animations** (subtle, one-shot, none loop): a small SVG car streaks across the top while a race loads (650ms, runs concurrently with the fetch, never delays it); the Gap Trace line draws itself left-to-right via `stroke-dashoffset` when its module is opened; Rejoin Strip dots pop in one by one in lap order (45ms stagger). All three check `prefers-reduced-motion` explicitly in JS rather than relying only on the global CSS override, so there's no single-frame flash for users who need it.

- Acceptance: the Glance layer fits one phone screen for any backfilled race; every summary number matches the module it summarizes when checked against the raw JSON (not just the rendering code); tapping any card or dot reveals the right content and nothing else stays open.
